...
/**
 * Requêtes filtrées et paginées pour les événements.
 */
final class EventListQueryService {
  ...
  private function applyFilters($query, array $filters): void {
    ...
    if ($keyword !== '') {
      $group = $query->orConditionGroup()
        ...
        ->condition('field_organizer', '%' . $keyword . '%', 'LIKE');
      $query->condition($group);
    }

    $status = trim($filters['status'] ?? '');
    if ($status !== '') {
      $query->condition('field_status', $status);
    }

    $this->applyTermFilter($query, 'field_type_evenement', 'type_evenement', $filters['type'] ?? '');
